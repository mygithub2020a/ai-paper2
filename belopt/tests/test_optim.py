...
class TestBelOpt(unittest.TestCase):
    ...
    def test_scheduler(self):
        ...
        lr_scheduler = CosineAnnealing(optimizer, 'lr', T_max=10)
        gamma_scheduler = CosineAnnealing(optimizer, 'gamma', T_max=10)
        beta_scheduler = CosineAnnealing(optimizer, 'beta', T_max=10)

        initial_lr = optimizer.param_groups[0]['lr']
        initial_gamma = optimizer.param_groups[0]['gamma']
        initial_beta = optimizer.param_groups[0]['beta']

        # Schedulers are typically called after an epoch, so the first value isn't used until after the first step
        lr_scheduler.step()
